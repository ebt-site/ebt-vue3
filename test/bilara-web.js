(typeof describe === 'function') && describe("bilara-web", function () {
  ...
  it("loadSuttaRef(...) => thig1.1 (en) refLang:de", async () => {
    ...
    let seg0 = sutta.segments[0];
    should(seg0.scid).match(/thig1.1:0.1/);
    should(seg0.pli).equal(PLI_THIG1_1);
    should(seg0.ref).match(/Strophen der altehrwürdigen Nonnen 1.1/);
    should(seg0.en).match(VERSES_SENIOR_NUNS);
  });
  ...
  it("TESTTESloadSuttaRef(...) => thig1.1 (soma) refLang:en", async () => {
    let bw = new BilaraWeb({ fetch });
    //bw.logLevel = 'info';
    let author = 'soma';
    let lang = 'en';
  ...
  it("loadSuttaRef(...) => thig1.1 (de)", async () => {
    ...
    should(seg0.scid).match(/thig1.1:0.1/);
    should(seg0.pli).equal(PLI_THIG1_1);
    should(seg0.de).match(/Strophen der altehrwürdigen Nonnen 1.1/);
  });
